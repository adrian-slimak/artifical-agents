...
namespace TerrainGeneration {
    ...
    public class TerrainGenerator : MonoBehaviour {
        ...
        public TerrainData Generate () {
            ...
            GenerateColliders(terrainData.walkable, terrainData.tileCentres, numTilesPerLine);
            return terrainData;
        }

        void GenerateColliders(bool[,] walkable, Vector3[,] tileCentres, int numTilesPerLine)
        {
            GameObject borderColliders = new GameObject("BorderColliders");
            borderColliders.AddComponent<CompositeCollider2D>();
            GameObject emptyCollider = new GameObject("Collider");
            emptyCollider.AddComponent<BoxCollider2D>().size = new Vector2(1, 1);

            GameObject collider = Instantiate(emptyCollider, Vector2.left*0.5f + Vector2.up * numTilesPerLine / 2f, Quaternion.identity, borderColliders.transform);
            collider.GetComponent<BoxCollider2D>().size = new Vector2(1f, numTilesPerLine);

            collider = Instantiate(emptyCollider, Vector2.right*(numTilesPerLine + 0.5f) + Vector2.up * numTilesPerLine / 2f, Quaternion.identity, borderColliders.transform);
            collider.GetComponent<BoxCollider2D>().size = new Vector2(1f, numTilesPerLine);

            collider = Instantiate(emptyCollider, Vector2.down * 0.5f + Vector2.right * numTilesPerLine / 2f, Quaternion.identity, borderColliders.transform);
            collider.GetComponent<BoxCollider2D>().size = new Vector2(numTilesPerLine, 1f);

            collider = Instantiate(emptyCollider, Vector2.up * (numTilesPerLine + 0.5f) + Vector2.right * numTilesPerLine / 2f, Quaternion.identity, borderColliders.transform);
            collider.GetComponent<BoxCollider2D>().size = new Vector2(numTilesPerLine, 1f);
            ...
            for (int y = 0; y < numTilesPerLine; y++)
            {
                ...
                for (int x = 0; x < numTilesPerLine; x++)
                {
                    ...
                    {
                        float xPos = (tileCentres[end, y].x + tileCentres[start, y].x)/2f;
                        collider = Instantiate(emptyCollider, new Vector3(xPos, tileCentres[0, y].z, 0f), Quaternion.identity, borderColliders.transform);
                        collider.GetComponent<BoxCollider2D>().size = new Vector2((end-start+1f),1f);
                        start = -1;
                        end = -1;
                    }
                }
